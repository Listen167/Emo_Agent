<template>
  <div class="chat-journal" @wheel="handleChatWheel">
    <header class="journal-header">
      ...
      <div class="roll-charm" aria-label="今日胶片吊牌">
        <span class="roll-charm-string" aria-hidden="true"></span>
        <span class="roll-charm-pin" aria-hidden="true"></span>
        <div class="roll-charm-body">
          <div class="roll-charm-top">
            <span>FIELD TAG</span>
            <i aria-hidden="true"></i>
          </div>
          <strong>{{ todayRollDate }}</strong>
          <small class="roll-charm-status">{{ currentXiaoxiAvatar.label }} · 在线</small>
          <div class="roll-charm-grid">
            <span>
              <b>{{ xiaoxiModeShortText }}</b>
              <em>陪伴模式</em>
            </span>
            <span>
              <b>{{ longMemoryCount }}</b>
...
let nextAudioStartAt = 0
let queuedAudioChunks: Promise<void>[] = []
let streamedAudioPlayed = false
...
const xiaoxiModeShortText = computed(() => {
  const map: Record<string, string> = {
    warm: '温柔',
    coach: '教练',
    rational: '理性',
  ...
  return map[xiaoxiMode.value] || '温柔'
})
...
const todayRollDate = computed(() => {
  const [, month, day] = shanghaiDayFormatter.format(new Date()).split('-')
  return `${month}.${day}`
})
...
const selectAvatar = (key: XiaoxiAvatarKey | null) => {
  ...
  window.dispatchEvent(new CustomEvent('u-life-xiaoxi-avatar-changed', { detail: { key } }))
...
const streamChatMessage = (msg: string) =>
  new Promise<void>((resolve, reject) => {
    ...
    socket.onmessage = async event => {
      ...
      if (payload.type === 'start') {
        emotionLabel = syncUserMessage(payload)
      ...
      if (payload.type === 'audio_delta') {
        void queueAudioChunk(decodeBase64ToUint8Array(payload.data || ''), payload.codec || 'mp3')
        return
      }
      ...
      if (payload.type === 'done') {
        ...
        assistantMessage.content = payload.text || streamedText
        assistantMessage.ttsAudioUrl = payload.tts_audio_url
        assistantMessage.createdAt = payload.assistant_created_at || assistantMessage.createdAt
        currentAvatarKey.value = getAvatarKeyByEmotion(emotionLabel)
        await Promise.allSettled(queuedAudioChunks)
        stopAudioQueueOnly()
        if (!streamedAudioPlayed && payload.tts_audio_url) {
          void playAssistantAudio(payload.tts_audio_url)
        }
        socket.close()
        resolve()
        return
      ...
      if (payload.type === 'error') {
        socket.close()
        reject(new Error(payload.message || '流式对话失败'))
      }
    }

    socket.onerror = () => reject(new Error('流式连接失败'))
    socket.onclose = () => {
...
const queueAudioChunk = (bytes: Uint8Array, codec: string) => {
  const task = playAudioChunk(bytes, codec).finally(() => {
    queuedAudioChunks = queuedAudioChunks.filter(item => item !== task)
  })
  queuedAudioChunks.push(task)
  return task
}

const playAudioChunk = async (bytes: Uint8Array, codec: string) => {
  if (!bytes.byteLength) return
  const context = audioContext || new AudioContext()
  audioContext = context
  const mime = codec === 'wav' ? 'audio/wav' : 'audio/mpeg'
  const buffer = await new Blob([bytes], { type: mime }).arrayBuffer()
  const audioBuffer = await context.decodeAudioData(buffer.slice(0))
  const source = context.createBufferSource()
  source.buffer = audioBuffer
  source.connect(context.destination)
  const startAt = Math.max(context.currentTime + 0.03, nextAudioStartAt)
  source.start(startAt)
  nextAudioStartAt = startAt + audioBuffer.duration
  audioSource = source
  streamedAudioPlayed = true
}

const stopAudioQueueOnly = () => {
  queuedAudioChunks = []
  nextAudioStartAt = 0
}
...
.roll-charm {
  ...
  animation: rollCharmSwing 5.8s ease-in-out infinite;
}
...
.roll-charm-string {
  ...
  width: 1px;
  height: 48px;
  background: linear-gradient(180deg, rgb(62 50 40 / 10%), rgb(62 50 40 / 34%));
...
.roll-charm-pin {
  ...
  width: 18px;
  height: 18px;
  border: 3px solid rgb(255 248 232 / 88%);
  border-radius: 999px;
  background: #c85a54;
  box-shadow:
    0 3px 7px rgb(62 50 40 / 18%),
    0 0 0 5px rgb(200 90 84 / 14%);
  animation: rollCharmPinPulse 2.5s ease-in-out infinite;
}
...
.roll-charm-body {
  ...
  text-align: center;
  background:
    linear-gradient(180deg, rgb(255 252 242 / 92%), rgb(242 218 154 / 86%)),
    repeating-linear-gradient(90deg, rgb(62 50 40 / 16%) 0 5px, transparent 5px 13px);
  box-shadow:
    inset 0 1px 0 rgb(255 255 255 / 76%),
    0 16px 30px rgb(62 50 40 / 15%);
}

.roll-charm-body::before {
  content: "";
  position: absolute;
  inset: 0;
  background: linear-gradient(115deg, transparent 0 30%, rgb(255 255 255 / 52%) 46%, transparent 62%);
  opacity: 0.72;
  transform: translateX(-90%);
  animation: rollCharmSweep 4.6s ease-in-out infinite;
  pointer-events: none;
}

.roll-charm-body::after {
  content: "";
  position: absolute;
  inset: 7px;
...
.roll-charm-top i {
  ...
  border-radius: 999px;
  background: #87a777;
  box-shadow: 0 0 0 4px rgb(135 167 119 / 16%), 0 0 12px rgb(135 167 119 / 48%);
  animation: xiaoxiStatusLight 2.4s ease-in-out infinite;
}

.roll-charm-body > strong {
  position: relative;
  z-index: 1;
  display: block;
  margin-top: 7px;
...
@keyframes rollCharmPinPulse {
  ...
  50% {
    ...
    box-shadow:
      0 5px 10px rgb(62 50 40 / 2%),
      0 0 0 8px rgb(200 90 84 / 7%);
  }
}

@keyframes rollCharmSweep {
  0%,
  22% {
