<template>
  <div class="growth-center">
    <header v-develop class="growth-header">
      ...
      <div class="growth-desk-prop" aria-hidden="true">
        <div class="weekly-folder-prop">
          <span class="folder-tab">WEEKLY REPORT</span>
          <span class="folder-paper paper-one"></span>
          <span class="folder-paper paper-two"></span>
        </div>
        <div class="memory-bottle-prop">
          <span class="bottle-neck"></span>
          <span class="bottle-glass"></span>
          <i class="capsule capsule-one"></i>
          <i class="capsule capsule-two"></i>
          <i class="capsule capsule-three"></i>
        </div>
      </div>
    </header>
    ...
    <main class="growth-layout">
      ...
      <aside class="growth-side">
        <section v-develop="100" class="persona-panel">
          ...
          <h2>小曦回应风格</h2>
          <p class="persona-hint">这里决定小曦怎么回应你；头像表情形象在首页选择。</p>
          <div class="current-avatar-card">
            <img :src="currentAvatar.src" :alt="currentAvatar.alt" />
            <div>
              <strong>{{ currentAvatar.label }}</strong>
              <small>当前小曦形象</small>
            </div>
          </div>
...
const lastAddedMemoryId = ref<string | null>(null)
...
const xiaoxiAvatars = {
  usual: { src: '/xiaoxi/usual.png', label: '日常陪伴', alt: '小曦日常表情' },
  happy: { src: '/xiaoxi/happy.png', label: '开心回应', alt: '小曦开心表情' },
  comfort: { src: '/xiaoxi/comfort.png', label: '温柔安慰', alt: '小曦安慰表情' },
  angry: { src: '/xiaoxi/angry.png', label: '认真站队', alt: '小曦生气表情' },
  shy: { src: '/xiaoxi/shy.png', label: '害羞惊喜', alt: '小曦害羞表情' },
  think: { src: '/xiaoxi/think.png', label: '认真思考', alt: '小曦思考表情' },
  naughty: { src: '/xiaoxi/naughty.png', label: '俏皮互动', alt: '小曦俏皮表情' },
} as const
...
const currentAvatar = computed(() => xiaoxiAvatars[currentAvatarKey.value])
...
function addMemory() {
  ...
  lastAddedMemoryId.value = nextId
  if (memoryAnimationTimer) window.clearTimeout(memoryAnimationTimer)
...
function setPersonality(value: string) {
...
.weekly-folder-prop {
  ...
  background:
    ...
    linear-gradient(180deg, #f2d98a, #d9a94e);
  box-shadow: 0 16px 28px rgb(62 50 40 / 16%);
  transform: rotate(-4deg);
}

.weekly-folder-prop::before {
  content: "";
  position: absolute;
  left: 12px;
  top: -13px;
...
.folder-paper {
  ...
  box-shadow: 0 6px 12px rgb(62 50 40 / 8%);
}
...
.bottle-glass {
  ...
  background:
    linear-gradient(120deg, rgb(255 255 255 / 48%), transparent 32%),
    rgb(253 251 247 / 42%);
  box-shadow:
    inset 0 0 0 1px rgb(255 255 255 / 48%),
    0 14px 24px rgb(62 50 40 / 14%);
...
.capsule {
  ...
  box-shadow: 0 4px 8px rgb(62 50 40 / 10%);
  animation: memoryCapsuleFloat 3.6s ease-in-out infinite;
}

.capsule-one {
  left: 21px;
  bottom: 42px;
  background: linear-gradient(90deg, #e8c36c 0 50%, #fff8e8 50% 100%);
  rotate: -15deg;
}
...
.capsule-two {
  left: 15px;
  bottom: 22px;
  background: linear-gradient(90deg, #c85a54 0 50%, #fff8e8 50% 100%);
  rotate: 16deg;
...
.capsule-three {
  right: 13px;
  bottom: 30px;
  background: linear-gradient(90deg, #87a777 0 50%, #fff8e8 50% 100%);
  rotate: -4deg;
...
.current-avatar-card img {
  width: 54px;
  height: 54px;
  object-fit: contain;
  filter: drop-shadow(0 6px 9px rgb(62 50 40 / 14%));
  animation: currentAvatarFloat 3.4s ease-in-out infinite;
}

.current-avatar-card strong,
.current-avatar-card small {
  display: block;
}
