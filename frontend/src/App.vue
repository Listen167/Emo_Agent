...
const syncOnboardingProfile = async () => {
  const rawSetup = localStorage.getItem('u-life-user-setup-v1')
  if (!rawSetup) return
  ...
  try {
    ...
    // Local onboarding should not block entering the app if profile sync fails.
  }
}
...
<template>
  ...
  <OnboardingModal v-if="!showLanding && showOnboarding" @complete="completeOnboarding" />
  ...
  <div v-if="!showLanding" :class="['journal-shell', 'paper-texture', { 'sidebar-collapsed': sidebarCollapsed }]">
    <aside class="journal-sidebar">
      <div class="sidebar-brand">
        <div class="brand-row">
          <button
            :class="['sidebar-paddle', 'sidebar-toggle', { collapsed: sidebarCollapsed }]"
            type="button"
          ...
          >
            ...
            <span
              :class="['paddle-grip', sidebarCollapsed ? 'grip-expand' : 'grip-collapse']"
              aria-hidden="true"
            ></span>
          </button>
        </div>
...
.sidebar-paddle {
  ...
  border-radius: 14px;
  background:
    linear-gradient(145deg, rgb(255 248 232 / 78%), rgb(255 255 255 / 34%)),
    rgb(232 195 108 / 62%);
  border: 1px solid rgb(62 50 40 / 16%);
  box-shadow:
    inset 0 1px 1px rgb(255 255 255 / 72%),
    inset 0 -10px 18px rgb(62 50 40 / 8%),
...
.sidebar-paddle::before {
  ...
  border-radius: 999px;
  background:
    radial-gradient(circle, rgb(62 50 40 / 34%) 0 2px, transparent 2px 6px) 0 0 / 4px 8px repeat-y;
  opacity: 0.72;
}
...
.sidebar-glyph {
  ...
  width: 17px;
  height: 20px;
  border: 2px solid rgb(62 50 40 / 66%);
  border-radius: 5px;
  background: rgb(255 248 232 / 54%);
  box-shadow: inset 5px 0 0 rgb(62 50 40 / 18%);
}
...
.paddle-grip {
  ...
  border-radius: 999px;
  background:
    linear-gradient(180deg, rgb(255 255 255 / 66%), rgb(62 50 40 / 10%)),
    rgb(253 251 247 / 58%);
  box-shadow:
    inset 0 1px 1px rgb(255 255 255 / 64%),
    0 2px 5px rgb(62 50 40 / 10%);
...
.sidebar-paddle::after {
  ...
  box-shadow: 0 10px 22px rgb(62 50 40 / 14%);
  font-size: 12px;
  font-weight: 800;
  opacity: 0;
  pointer-events: none;
...
.sidebar-paddle:hover::after,
.sidebar-paddle:focus-visible::after {
  opacity: 1;
...
.sidebar-toggle:hover {
  ...
  box-shadow:
    inset 0 1px 1px rgb(255 255 255 / 72%),
    inset 0 -12px 22px rgb(62 50 40 / 8%),
    0 11px 24px rgb(62 50 40 / 22%);
}

.sidebar-toggle:hover .paddle-grip {
  background:
    linear-gradient(180deg, rgb(255 255 255 / 76%), rgb(62 50 40 / 8%)),
    rgb(255 248 232 / 72%);
  transform: translateX(2px);
}

.sidebar-toggle:hover .grip-expand {
...
.sidebar-collapsed .sidebar-paddle {
  width: 56px;
  height: 40px;
  justify-content: center;
  gap: 7px;
...
.sidebar-collapsed .sidebar-paddle::before {
  opacity: 0;
}

.sidebar-collapsed .sidebar-glyph {
  width: 16px;
  height: 20px;
}
